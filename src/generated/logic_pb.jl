...
function meta(::Type{LogicManager_Req})
    ProtoBuf.metalock() do
        if !isassigned(__meta_LogicManager_Req)
            ...
            allflds = Pair{Symbol,Union{Type,String}}[:get_rule_req => LogicManager_GetRule_Req, :put_rule_req => LogicManager_PutRule_Req]
            oneofs = Int[1,1]
